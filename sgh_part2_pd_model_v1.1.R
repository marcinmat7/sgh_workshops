...
# Labels (credit_records)
    ...
    credit_records$def[credit_records$STATUS >= 3] <- 1
...
# Labels (credit_records)
    ...
    for(i in unique(app_record2$CODE_GENDER)) {
        print(i)
        table(credit_records3$def[app_record2$CODE_GENDER == i])
    }
...
table(data2$FLAG_OWN_CAR)
data2 <- add_logits(data2, "FLAG_OWN_CAR")

table(data2$FLAG_OWN_REALTY)
data2 <- add_logits(data2, "FLAG_OWN_REALTY")

data2 <- add_logits(data2, "NAME_INCOME_TYPE")
table(data2$NAME_EDUCATION_TYPE)
data2 <- add_logits(data2, "NAME_EDUCATION_TYPE")

table(data2$NAME_FAMILY_STATUS)
data2 <- add_logits(data2, "NAME_FAMILY_STATUS")

table(data2$NAME_HOUSING_TYPE)
data2 <- add_logits(data2, "NAME_HOUSING_TYPE")

table(data2$DAYS_BIRTH)
table(data2$DAYS_EMPLOYED)
table(data2$FLAG_MOBIL, exclude = NULL)
table(data2$FLAG_WORK_PHONE, exclude = NULL)
data2 <- add_logits(data2, "FLAG_WORK_PHONE")

table(data2$FLAG_PHONE, exclude = NULL)
data2 <- add_logits(data2, "FLAG_PHONE")

table(data2$FLAG_EMAIL, exclude = NULL)
data2 <- add_logits(data2, "FLAG_EMAIL")

table(data2$OCCUPATION_TYPE, exclude = NULL)
data2 <- add_logits(data2, "OCCUPATION_TYPE")

table(data2$CNT_FAM_MEMBERS, exclude = NULL)
...
# Model 1
    ...
    for(i in names(data3_train)) {
        data3_train[[i]][is.infinite(data3_train[[i]])] <- 
            10
    }
...
# # Model 2 (oversampling)
    ...
    for(i in names(data3_train)) {
        data3_train[[i]][is.infinite(data3_train[[i]])] <- 10
    }
...
# Model 3 (num featueres standardised)
